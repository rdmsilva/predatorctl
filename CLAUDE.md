...
`install.sh` installs and enables the unit, plus the `/etc/predatorctl/` dir and `restore.conf.example` — but never creates `restore.conf` itself, and `ConditionPathExists=/etc/predatorctl/restore.conf` on the unit means "enabled" is a no-op (successful, `RemainAfterExit`) until the user creates that file. This is safe to default-enable specifically *because* of that condition gate plus the file being root-only-writable: an unprivileged user can neither trigger the unit nor supply what it reads, so enabling by default doesn't move the privilege boundary — it only removes a manual step. Opting in is then just creating the file (`cp restore.conf.example restore.conf`, edit, `systemctl restart` to apply without waiting for a reboot).
